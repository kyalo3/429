...
export default function DonorRegistrationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Step 3: Create the donor profile
      const donorResponse = await fetch("http://localhost:8000/donors/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          services_interested_in: formData.services_interested_in,
          participating_locations: formData.participating_locations,
          type_of_company: formData.type_of_company,
        }),
      });

      if (!donorResponse.ok) {
        throw new Error("Donor profile creation failed");
      }
